...
public class Branch {
	 ...
	 * @return the tag list
	 */
	public TagList 			getTagList() 		{ return theTags; }
	...
	private Branch(Component	pParent,
		...
		if (myParts.length != 3) throw new IllegalArgumentException();
	...
	public SVNURL getURL() {
		/* Build the URL */
		try { return SVNURL.parseURIDecoded(getPath()); }
	...
	public int compareTo(Branch pThat) {
        ...
        if (theMajorVersion < pThat.theMajorVersion) return -1;
        if (theMajorVersion > pThat.theMajorVersion) return 1;
        if (theMinorVersion < pThat.theMinorVersion) return -1;
        if (theMinorVersion > pThat.theMinorVersion) return 1;
        if (theRevision     < pThat.theRevision)     return -1;
        if (theRevision     > pThat.theRevision)     return 1;
        return 0;
	}
	...
	public static class BranchList {
		 ...
		 */
		public List<Branch>		getList() 	{ return theList; }
		...
		public void discover() throws ModelException {
			...
			try { 
				...
				throw new ModelException(ExceptionClass.SUBVERSION,
										 "Failed to discover branches for " + theComponent.getName(),
										 e);
			}
		}
		...
		public Branch nextMinorBranch(Branch pBase) {
			...
			while (myIterator.hasNext()) {
				...
				if (myTest.theMajorVersion > myMajor) break;
				if (myTest.theMajorVersion < myMajor) continue;
		...
		public Branch nextRevisionBranch(Branch pBase) {
			...
			while (myIterator.hasNext()) {
				...
				if (myTest.theMajorVersion > myMajor) break;
				if (myTest.theMajorVersion < myMajor) continue;
				if (myTest.theMinorVersion > myMinor) break;
				if (myTest.theMinorVersion < myMinor) continue;
		...
		private class BranchHandler implements ISVNDirEntryHandler {
			...
			public void handleDirEntry(SVNDirEntry pEntry) throws SVNException {
				...
				try { 
					...
					if (pEntry.getKind() != SVNNodeKind.DIR) return;
					...
					if (!myName.startsWith(branchPrefix)) return;
