...
public class Tag {
	...
	public int compareTo(Tag pThat) {
        ...
        /* Compare tag numbers */
        if (theTag > pThat.theTag) return -1;
        if (theTag < pThat.theTag) return 1;
        return 0;
	}
	...
	public static class TagList {
		 ...
		 * Obtain the tag list
		 */
		public List<Tag>	getList() 	{ return theList; }
		...
		public void discover() throws ModelException {
			...
			try { 
				...
				throw new ModelException(ExceptionClass.SUBVERSION,
										 "Failed to discover tags for " + theBranch.getBranchName(),
										 e);
			}
		}
		...
		private class TagHandler implements ISVNDirEntryHandler {
			...
			public void handleDirEntry(SVNDirEntry pEntry) throws SVNException {
				...
				if (!myName.startsWith(thePrefix)) return;
