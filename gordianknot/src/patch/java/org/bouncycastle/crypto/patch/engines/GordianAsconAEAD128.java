package org.bouncycastle.crypto.patch.engines;
 ...
 * @version 1.3
 */
public class GordianAsconAEAD128
        extends GordianAsconBaseEngine
{
    public GordianAsconAEAD128()
    {
        CRYPTO_KEYBYTES = 16;
        CRYPTO_ABYTES = 16;
        ASCON_AEAD_RATE = 16;
    ...
    {
        return GordianPack.littleEndianToLong(in, inOff);
    }

    @Override
    protected void setBytes(long n, byte[] bs, int off)
    {
        GordianPack.longToLittleEndian(n, bs, off);
    }
    ...
    protected void processFinalAadBlock()
    {
        ...
        {
            x0 ^= GordianPack.littleEndianToLong(m_buf, 0);
            x1 ^= GordianPack.littleEndianToLong(m_buf, 8) ^ pad(m_bufPos);
        }
        else
        {
            x0 ^= GordianPack.littleEndianToLong(m_buf, 0) ^ pad(m_bufPos);
        }
    }
    ...
    protected void processFinalDecrypt(byte[] input, int inLen, byte[] output, int outOff)
    {
        ...
        {
            long c0 = GordianPack.littleEndianToLong(input, 0);
            inLen -= 8;
            long c1 = GordianPack.littleEndianToLong(input, 8, inLen);
            GordianPack.longToLittleEndian(x0 ^ c0, output, outOff);
            GordianPack.longToLittleEndian(x1 ^ c1, output, outOff + 8, inLen);
            x0 = c0;
            x1 &= -(1L << (inLen << 3));
            x1 |= c1;
            x1 ^= pad(inLen);
        ...
        else
        {
            if (inLen != 0)
            {
                long c0 = GordianPack.littleEndianToLong(input, 0, inLen);
                GordianPack.longToLittleEndian(x0 ^ c0, output, outOff, inLen);
                x0 &= -(1L << (inLen << 3));
                x0 |= c0;
            }
            x0 ^= pad(inLen);
    ...
    protected void processFinalEncrypt(byte[] input, int inLen, byte[] output, int outOff)
    {
        if (inLen >= 8) // ASCON_AEAD_RATE == 16 is implied
        {
            x0 ^= GordianPack.littleEndianToLong(input, 0);
            inLen -= 8;
            x1 ^= GordianPack.littleEndianToLong(input, 8, inLen);
            GordianPack.longToLittleEndian(x0, output, outOff);
            GordianPack.longToLittleEndian(x1, output, outOff + 8);
            x1 ^= pad(inLen);
        }
        else
        {
            if (inLen != 0)
            {
                x0 ^= GordianPack.littleEndianToLong(input, 0, inLen);
                GordianPack.longToLittleEndian(x0, output, outOff, inLen);
            }
            x0 ^= pad(inLen);
        }
            ...
            throws IllegalArgumentException
    {
        ...
        K0 = GordianPack.littleEndianToLong(k, 0);
        K1 = GordianPack.littleEndianToLong(k, 8);
        N0 = GordianPack.littleEndianToLong(npub, 0);
        N1 = GordianPack.littleEndianToLong(npub, 8);
