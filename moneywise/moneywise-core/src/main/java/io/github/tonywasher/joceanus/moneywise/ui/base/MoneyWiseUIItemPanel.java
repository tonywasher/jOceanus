 ...
 */
public abstract class MoneyWiseUIItemPanel<T extends PrometheusDataItem>
        extends PrometheusDataItemPanel<T, MoneyWiseUIGoToId> {
    /**
     * Filter text.
     */
     ...
     * The Owning table.
     */
    private final MoneyWiseUIBaseTableCtl<T> theOwner;
     ...
     */
    protected MoneyWiseUIItemPanel(final TethysUIFactory<?> pFactory,
                                   final PrometheusEditSet pEditSet,
                                   final MoneyWiseUIBaseTableCtl<T> pOwner) {
     ...
     * @return the owner
     */
    protected MoneyWiseUIBaseTableCtl<T> getOwner() {
        return theOwner;
    }
    ...
    private void processGoToItems(final TethysUIScrollMenu<TethysUIGenericWrapper> pMenu) {
        ...
        for (PrometheusDataItem myItem : theGoToItemList) {
            ...
            switch (myItem) {
                ...
                case MoneyWiseAssetBase myAccount -> {
                    myId = MoneyWiseUIGoToId.ACCOUNT;
                    myName = myAccount.getName();
                }
                case MoneyWiseCategoryBase myCategory -> {
                    myId = MoneyWiseUIGoToId.CATEGORY;
                    myName = myCategory.getName();
                }
                case MoneyWiseRegion myRegion -> {
                    myId = MoneyWiseUIGoToId.REGION;
                    myName = myRegion.getName();
                }
                case MoneyWiseTransTag myTag -> {
                    myId = MoneyWiseUIGoToId.TAG;
            ...
            /* Build the item */
            final PrometheusGoToEvent<MoneyWiseUIGoToId> myEvent = createGoToEvent(myId, myItem);
    ...
    private void processGoToFilters(final TethysUIScrollMenu<TethysUIGenericWrapper> pMenu) {
        ...
        for (MoneyWiseAnalysisFilter<?, ?> myFilter : theGoToFilterList) {
            ...
            final MoneyWiseUIGoToId myId = MoneyWiseUIGoToId.STATEMENT;

            /* Build the item */
            final PrometheusGoToEvent<MoneyWiseUIGoToId> myEvent = createGoToEvent(myId, myStatement);
