...
import io.github.tonywasher.joceanus.moneywise.data.basic.MoneyWiseSecurityHoldingMap.MoneyWisePortfolioHoldingsMap;
 ...
 * SecurityHolding Map.
 */
public class MoneyWiseSecurityHoldingMap
        implements MetisDataObjectFormat, MetisDataMap<Integer, MoneyWisePortfolioHoldingsMap>, MoneyWiseSecurityHoldingMapCtl {
    /**
    ...
    private static String getPortfolioName(final String pName) {
        final int iIndex = pName.indexOf(MoneyWiseSecurityHolding.SECURITYHOLDING_SEP);
        return iIndex == -1
                ? pName
    ...
    @Override
    public void deRegister(final MoneyWiseAssetBase pAsset) {
        /* Ensure that we do not reference this asset */
        switch (pAsset) {
            case MoneyWisePortfolio myPortfolio -> deRegister(myPortfolio);
            case MoneyWiseSecurity mySecurity -> deRegister(mySecurity);
            default -> {
                /* NoOp */
            }
        }
     ...
     * @param pPortfolio the portfolio
     */
    private void deRegister(final MoneyWisePortfolio pPortfolio) {
     ...
     * @param pSecurity the security
     */
    private void deRegister(final MoneyWiseSecurity pSecurity) {
        /* Access the id */
        final Integer myId = pSecurity.getIndexedId();
