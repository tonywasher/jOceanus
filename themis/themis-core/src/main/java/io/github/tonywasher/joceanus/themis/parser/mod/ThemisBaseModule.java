 ...
 * @param <T> the Node
 */
public abstract class ThemisBaseModule<T extends Node>
        extends ThemisBaseInstance<T>
        implements ThemisModuleInstance {
    /**
     * The nodeId.
     */
    private final ThemisMod theId;
     ...
     */
    ThemisBaseModule(final ThemisParserDef pParser,
                     final T pNode) throws OceanusException {
        super(pParser, pNode);
        theId = ThemisMod.determineModule(pParser, pNode);
    }

    @Override
    public ThemisMod getId() {
        return theId;
    }
}
