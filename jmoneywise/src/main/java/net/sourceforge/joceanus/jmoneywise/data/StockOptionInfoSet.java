...
/**
 * StockOptionInfoSet class.
 * @author Tony Washer
 */
public class StockOptionInfoSet
        extends DataInfoSet<StockOptionInfo, StockOption, AccountInfoType, AccountInfoClass, MoneyWiseDataType> {
    /**
     * Report fields.
     */
    private static final JDataFields FIELD_DEFS = new JDataFields(MoneyWiseDataResource.STOCKOPTION_INFOSET.getValue(), DataInfoSet.FIELD_DEFS);
     ...
     */
    protected StockOptionInfoSet(final StockOption pOwner,
                                 final AccountInfoTypeList pTypeList,
                                 final StockOptionInfoList pInfoList) {
     ...
     */
    protected void cloneDataInfoSet(final StockOptionInfoSet pSource) {
        /* Clone the dataInfoSet */
    ...
    protected void validate() {
        /* Access details about the Option */
        StockOption myOption = getOwner();

        /* Loop through the classes */
        for (AccountInfoClass myClass : AccountInfoClass.values()) {
            /* Access info for class */
            StockOptionInfo myInfo = getInfo(myClass);
            ...
            if (!isExisting) {
                ...
                if (myState == JDataFieldRequired.MUSTEXIST) {
                    myOption.addError(DataItem.ERROR_MISSING, getFieldForClass(myClass));
                }
                continue;
            }
            ...
            if (myState == JDataFieldRequired.NOTALLOWED) {
                myOption.addError(DataItem.ERROR_EXIST, getFieldForClass(myClass));
                continue;
            }
            ...
            switch (myClass) {
                case NOTES:
                    ...
                    if (myArray.length > myClass.getMaximumLength()) {
                        myOption.addError(DataItem.ERROR_LENGTH, getFieldForClass(myClass));
                    }
                    break;
                default:
                    break;
