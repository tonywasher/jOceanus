...
public class TableDeposit
        extends PrometheusXTableEncrypted<Deposit> {
    /**
     ...
     */
    protected TableDeposit(final PrometheusXDataStore pDatabase) {
        super(pDatabase, TABLE_NAME);
        final PrometheusXTableDefinition myTableDef = getTableDef();

        /* Declare the columns */
        final PrometheusXColumnDefinition myCatCol = myTableDef.addReferenceColumn(Deposit.FIELD_CATEGORY, TableDepositCategory.TABLE_NAME);
    ...
    protected DataValues loadValues() throws OceanusException {
        ...
        /* Build data values */
        final DataValues myValues = getRowValues(Deposit.OBJECT_NAME);
