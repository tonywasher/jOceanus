...
public abstract class SheetDataItem<T extends DataItem> {
	 ...
	 * The input sheet
	 */
	private InputSheet 			theInputSheet	= null;
	 ...
	 *  @param pWorkbook the workbook to read from
	 *  @param pRange 	 the range to load
	 */
	protected SheetDataItem(InputSheet 	pInput,
							String		pRange) {
		/* Store parameters */
		theThread 		= pInput.getThread();
		theInputSheet 	= pInput;
		theRangeName	= pRange;
	}

	/**
	 * Constructor for a write operation
	 *  @param pOutput   the output sheet
	 *  @param pRange 	 the range to create
	 */
	protected SheetDataItem(OutputSheet		pOutput,
							String			pRange) {
		/* Store parameters */
		theThread 		= pOutput.getThread();
		theWriteBook 	= pOutput.getWorkBook();
		theRangeName	= pRange;
	}
	...
	public boolean loadSpreadSheet() throws Exception {
		...
		try { 
			/* Access the workbook */
			theWorkBook = theInputSheet.getWorkBook();
