 ...
 * $URL: http://localhost/svn/Finance/jOceanus/trunk/jmetis/jmetis-core/src/main/java/net/sourceforge/joceanus/jmetis/service/sheet/jopen/MetisJOpenFactory.java $
 * $Revision: 923 $
 * $Author: Tony $
...
/**
 * Factory to load/initialise an Oasis jOpenDocument WorkBook.
 */
@MetisSheetService(MetisSheetWorkBookType.OASISJOPEN)
public class MetisJOpenFactory
        implements MetisSheetFactory {
    @Override
    public MetisSheetWorkBook loadFromStream(final InputStream pInput) throws OceanusException {
        return new MetisJOpenWorkBook(pInput);
    }

    @Override
    public MetisSheetWorkBook newWorkBook() throws OceanusException {
        return new MetisJOpenWorkBook();
    }
}
