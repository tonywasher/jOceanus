 ...
 */
public class MetisOrderedIndex<T extends Comparable<? super T>> {
    /**
    ...
    protected MetisOrderedIndex(final int pIndexGranularity) {
        /* Reject if granularity is out of range */
        if ((pIndexGranularity < MetisOrderedIndex.MIN_GRANULARITY_SHIFT)
            || (pIndexGranularity > MetisOrderedIndex.MAX_GRANULARITY_SHIFT)) {
     ...
     */
    protected void declareList(final MetisOrderedList<T> pList) {
        theList = pList;
    }
    ...
    protected MetisOrderedNode<T> findNodeForObject(final T pItem) {
        ...
        MetisOrderedNode<T> myTest = theMap[iMinimum];
        ...
        if (iMaximum > 0) {
            ...
            if (iDiff == 0) {
                ...
                /*
                 * Binary chop to find the search start point. We need to loop while we have a
                 * search span greater than granularity
                 */
    ...
    /**
     * Obtain the node for the specified item. This slow method is used on the reSort method where
     * we expect the item to be outside its natural ordering and hence not found by the standard
     * search.
     * @param pItem the item
     * @return the relevant node (or null)
     */
    protected MetisOrderedNode<T> findUnsortedNodeForObject(final T pItem) {
        ...
        MetisOrderedNode<T> myNode = theList.getFirst();
     ...
     */
    protected MetisOrderedNode<T> findNodeAfter(final MetisOrderedNode<T> pNode) {
    ...
    /**
     * Register link between object and node to allow fast lookup of node from object. Standard
     ...
     * @param pNode the node to insert
     */
    protected void insertNode(final MetisOrderedNode<T> pNode) {
    ...
    protected void removeNode(final MetisOrderedNode<T> pNode) {
        ...
        /*
         * Determine the active map length (note that list is one too large since we have yet to
         * remove this item)
         */
