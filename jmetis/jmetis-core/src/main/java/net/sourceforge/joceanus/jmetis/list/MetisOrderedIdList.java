...
/**
 * Ordered Id list. This provides an improved ordered list implementation for items that have a
 * non-mutable identity, and is the preferred implementation. Care should be taken to ensure that
 * the identity is part of the natural ordering to ensure consistency.
 * <p>
 * The identity is used to directly look up the linked list node for the item via a HashMap. This
 * provides significant performance improvements over {@link MetisOrderedList}, and also solves the
 * problems associated with <b>dirty</b> lists.
 * <ol>
 * <li>{@link #remove(Object)}, {@link #contains} and {@link #indexOf} are all accurate on a dirty
 * list.
 * <li>{@link #add} will reject entries for which the identity is already in the list
 * </ol>
 * <p>
 * The {@link #add} method will still add objects in a best can do fashion, and the {@link #reSort}
 * method should still be used to repair a dirty list.
 * @author Tony Washer
...
public class MetisOrderedIdList<I, T extends Comparable<? super T> & MetisOrderedIdItem<I>>
     ...
     */
    public MetisOrderedIdList(final MetisOrderedIdList<I, T> pSource) {
    ...
    private MetisOrderedIdIndex<I, T> getTheIndex() {
        return (MetisOrderedIdIndex<I, T>) super.getIndex();
    }
    ...
    /**
     * Add item into correct sort order in the list. If the list is not currently sorted, then the
     * item will not be correctly placed into the list. If the item has the same Id as an existing
     * item in the list it will not be added.
     ...
     * operation. If the item has the same Id as an existing item in the list it will not be added.
     * @param pItem the item to add
     * @return true if the item was added to the list.
     */
    @Override
